...
export function Level1UserDashboard({
  ...
  const [activeTab, setActiveTab] = useState<"departments" | "requests">(
    "departments",
  );
  const [requestTypeTab, setRequestTypeTab] = useState<
    "all" | "generic" | "specific"
  >("generic"); // Default to generic for priority
  ...
  const fetchDepartments = async () => {
    try {
      ...
      console.log(
        `Fetching departments from: ${import.meta.env.VITE_API_URL}/api/requests/departments-with-assets`,
      );
      // ✅ CORRECT
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/departments-with-assets`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        console.log(
          ...
          data.data?.[0] ? Object.keys(data.data[0]) : "No departments",
        );
        ...
        if (data.data && data.data.length > 0) {
          ...
          const departmentsWithIds = data.data.map((dept: any) => ({
            ...dept,
            _id:
              dept._id || dept.id || dept.departmentId || `dept-${dept.code}`,
          }));
  ...
  const fetchDepartmentAssets = async (departmentId: string) => {
    ...
    try {
      ...
      console.log(
        `Fetching assets for department ${departmentId} from: ${url}`,
      );
  ...
  const requestSpecificAsset = async (asset: Asset) => {
    ...
    if (
      ...
      alert(
        "You cannot request assets from your own department. Please select a different department.",
      );
      return;
    }
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/specific-assets`,
        {
          method: "POST",
          headers: {
  ...
  const fetchAssets = async () => {
    try {
      const token = localStorage.getItem("accessToken");
      console.log(
        `Fetching assets from: ${import.meta.env.VITE_API_URL}/api/requests/assets/department`,
      );
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/assets/department`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchRequests = async () => {
    try {
      const token = localStorage.getItem("accessToken");
      console.log(
        `Fetching open requests from: ${import.meta.env.VITE_API_URL}/api/requests/open`,
      );
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/open`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.ok) {
        ...
        const filteredRequests = data.data.filter((request: any) => {
          ...
          console.log(
            "Asset Department Map keys:",
            Object.keys(request.assetDepartmentMap || {}),
          );
          console.log(
            "Asset Department Map generic_request:",
            request.assetDepartmentMap?.generic_request,
          );
          ...
          console.log(
            ...
            request.scope?.departmentId?._id,
          );
          console.log(
            "Request department name:",
            request.scope?.departmentId?.name || "Unknown",
          );
          ...
          if (isSpecificAssetRequest) {
            ...
            const assetDepartmentIds = Object.values(
              request.assetDepartmentMap || {},
            )
              .map((asset: any) => asset.departmentId)
              .filter(Boolean);

            console.log(
              "Specific request - asset department IDs:",
              assetDepartmentIds,
            );
            console.log("User department ID:", userDepartment);

            // Show request if user's department holds any of the requested assets
            const userHasRequestedAsset =
              assetDepartmentIds.includes(userDepartment);
        ...
        if (requestTypeTab === "generic") {
          finalFilteredRequests = filteredRequests.filter((req) => {
            ...
            console.log(
              "Generic tab - Request ID:",
              req._id,
              "Is generic:",
              isGeneric,
              "assetDepartmentMap:",
              req.assetDepartmentMap,
            );
            return isGeneric;
          });
        ...
        console.log(
          "Final filtered requests for tab:",
          requestTypeTab,
          finalFilteredRequests.length,
          finalFilteredRequests,
        );
  ...
  const fulfillRequest = async (requestId: string, assetIds: string[]) => {
    try {
      const token = localStorage.getItem("accessToken");
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/${requestId}/fulfill`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const rejectRequest = async (
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/${requestId}/reject-assets`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        fetchRequests();
        setSuccessMessage(
          `Successfully rejected ${result.rejectedAssets} asset(s)!`,
        );
        setShowSuccessDialog(true);
      } else {
        const error = await response.json();
  ...
  const updateAssetStatus = async (
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/requests/assets/${assetId}/utilization`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const getDepartmentName = (deptId: string) => {
    const dept = departments.find(
      (d) => d.departmentId === deptId || d._id === deptId,
    );
  ...
  const userDepartments = departments.filter(
    (dept) =>
      ...
      dept.code?.toLowerCase().trim() ===
        userDepartment?.toLowerCase().trim() ||
      dept.departmentId?.toLowerCase().trim() ===
        userDepartment?.toLowerCase().trim() ||
  ...
  const pendingRequests = requests.filter(
    (req) => req.finalStatus === "pending" || req.finalStatus === "escalated",
  );
            ...
            <Button
              onClick={() => {
                ...
                if (
                  ...
                  alert(
                    "You cannot create requests for your own department. Please select a different department to request assets from.",
                  );
                  return;
                }
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-600 mb-4">
                This is your own department. You can view assets but cannot
                request from your own department.
              </p>
              {userDepartments.length > 0 ? (
                ...
                <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <p className="text-gray-600">
                    No department data found for your department.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-600 mb-4">
                You can request assets from these departments. Click on any
                department to view and request their available assets.
              </p>
              {otherDepartments.length > 0 ? (
                ...
                <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <p className="text-gray-600">
                    No other departments available for requests.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {selectedDepartment.code} • {assets.length} Ideal Assets
                  </p>
                  {(selectedDepartment.name === userDepartment ||
                    selectedDepartment.code === userDepartment) && (
                    <Badge
              ...
              <CardHeader>
                ...
                {(selectedDepartment.name === userDepartment ||
                  ...
                  <p className="text-sm text-gray-600">
                    You can view these assets but cannot request from your own
                    department.
                  </p>
                )}
              </CardHeader>
                        ...
                        {filteredAssets
                          ...
                          .map((asset) => (
                            ...
                            >
                              ...
                              <td className="p-3">
                                <Badge
                                  className={`${getStatusColor(asset.utilizationStatus)} border`}
                                >
                                  <div className="flex items-center gap-1">
                                    {getStatusIcon(asset.utilizationStatus)}
                                    <span className="capitalize">
                                      {asset.utilizationStatus
                                        ? asset.utilizationStatus.replace(
                                            "_",
                                            " ",
                                          )
                                        : "Unknown"}
                              ...
                              <td className="p-3">
                                <div className="flex gap-2">
                                  <Select
                                    onValueChange={(value) =>
                                      updateAssetStatus(asset._id, value)
                                    }
                                  >
                ...
                <div className="flex gap-2">
                  ...
                  {requestTypeTab === "generic" && (
                    <Badge className="bg-blue-100 text-blue-700 border-blue-200">
                      ...
                      {
                        allRequests.filter(
                          (req) => req.assetDepartmentMap?.generic_request,
                        ).length
                      }
                    </Badge>
                  )}
                ...
                <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-blue-700">
                      Generic requests are shown first for immediate attention
                    </span>
                  </div>
                </div>
              )}
                    ...
                    {requests.map((request) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="space-y-2">
                              ...
                              {isGeneric ? (
                                <div className="text-sm text-blue-600">
                                  Generic request for {request.assetCategory}{" "}
                                  assets
                                </div>
                              ) : (
                              ...
                              <div className="text-xs text-gray-500">
                                From:{" "}
                                {request.scope?.departmentId?.name ||
                                  "Unknown Department"}
                              </div>
                            </div>
                          </td>
                          <td className="p-3">
                            <Badge
                              className={`${getPriorityColor(request.priority)} font-medium`}
                            >
                          ...
                          <td className="p-3">
                            <div className="flex items-center gap-2">
                              <div className="text-sm">
                                <span className="font-medium">
                                  {request.fulfillment?.fulfilledCount || 0}
                                </span>
                                <span className="text-gray-500">
                                  {" "}
                                  / {request.requestedAssets?.length || 1}
                                </span>
                              </div>
                          ...
                          <td className="p-3">
                            <div className="flex space-x-2">
                              <Button
                                ...
                                disabled={
                                  (request.fulfillment?.fulfilledCount || 0) > 0
                                }
                              >
                                Fulfill
                              </Button>
