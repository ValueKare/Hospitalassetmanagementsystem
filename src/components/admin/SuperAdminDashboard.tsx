...
import { Building2, Package, Plus, TrendingUp, AlertCircle, Building } from "lucide-react";
...
export const getActiveAudits = async () => {
  try {
    const token = getAuthToken();
    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/audit?status=in_progress&limit=10`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Failed to fetch active audits: ${response.status} ${errorText}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.error('Active audits API error:', error);
    throw error;
  }
};
...
export function SuperAdminDashboard({ onNavigate, selectedEntity }: SuperAdminDashboardProps) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const activeAuditsResponse = results[6].status === 'fulfilled' ? results[6].value : null;

      // Set active audits for the dashboard
      if (activeAuditsResponse?.audits) {
        setActiveAudits(activeAuditsResponse.audits);
      }
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`inline-block px-2 py-1 rounded-full text-xs ${alert.priority === 'high' ? 'bg-red-100 text-red-800' :
                    ...
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={(value: any, name: string) => {
                          ...
                          const numValue = typeof value === 'object' && value?.$numberDecimal
                            ? parseFloat(value.$numberDecimal)
                            : typeof value === 'number' ? value : 0;
                          return [numValue || 0, name || 'Unknown'];
                        }}
            ...
            <CardContent>
              <div className="space-y-4">
                {activeAudits.length === 0 ? (
                  ...
                  activeAudits.map((audit: ActiveAudit) => (
                    <div key={audit._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      <div className="flex-1">
                        <p className="text-gray-900">{audit.hospitalId?.name || 'Unknown Hospital'}</p>
                        <p className="text-gray-600">
                          {audit.stats?.totalAssets || 0} assets
                        </p>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="text-gray-900">{audit.stats?.verificationRate || 0}% Complete</p>
                        ...
                        <div className="w-32 bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{ width: `${audit.stats?.verificationRate || 0}%` }}
                          />
                        </div>
                      </div>
                    </div>
