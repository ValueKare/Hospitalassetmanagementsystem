...
export function AdminAssetManagement({ onNavigate, selectedEntity }: { onNavigate: (screen: string) => void; selectedEntity?: any }) {
  ...
  const fetchEntityHospitals = async () => {
    try {
      ...
      if (selectedEntity && selectedEntity.code) {
        ...
        const res = await fetch(`http://localhost:5001/api/dashboard/hospitals?entityCode=${selectedEntity.code}`, {
          headers: {
  ...
  const fetchAssets = async (page: number = 1) => {
    try {
      ...
      console.log('Fetching from:', `http://localhost:5001/api/sql/assets/paginated?${params}`);
      const res = await fetch(`http://localhost:5001/api/sql/assets/paginated?${params}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (json.success) {
        ...
        const fallbackRes = await fetch("http://localhost:5001/api/sql/assets/all", {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
